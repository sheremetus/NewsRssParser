...
async def main():
    ...
    async with client:
        ...
        limit = 200
        one_week_ago = datetime.now() - timedelta(days=7)
        msg_cnt = limit or sum([1 async for _ in client.iter_messages(d.id, offset_date=one_week_ago)])
        chapters = []
       
        msg_iter = client.iter_messages(d.id, reverse=True, limit=limit, offset_date=one_week_ago)
